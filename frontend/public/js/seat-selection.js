...
let bookedSeats = [];
let gitamEmail = '';
let seatsToBook = 1; // Limited to 1 seat per user
...
const SEATING_LAYOUT = [
  ...
  // Balcony Section
  { row: 'A', seats: 26, aisleAfter: 13, section: 'Balcony' },
  { row: 'B', seats: 26, aisleAfter: 13, section: 'Balcony' },
  { row: 'C', seats: 28, aisleAfter: 14, section: 'Balcony' },
  { row: 'D', seats: 28, aisleAfter: 14, section: 'Balcony' },
  { row: 'E', seats: 28, aisleAfter: 14, section: 'Balcony' },
  { row: 'F', seats: 28, aisleAfter: 14, section: 'Balcony' },
  { row: 'G', seats: 28, aisleAfter: 14, section: 'Balcony' },
  { row: 'H', seats: 28, aisleAfter: 14, section: 'Balcony' },
];
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // Set seats to select display
    document.getElementById('seats-to-select').textContent = seatsToBook;
    
    // Generate seat map directly (no quantity selection)
    generateSeatMap();
  } catch (error) {
...
async function loadBookedSeats() {
  try {
    ...
    console.log('Loaded booked seats:', bookedSeats);
  } catch (error) {
...
function generateSeatMap() {
  ...
  SEATING_LAYOUT.forEach((rowConfig, index) => {
    ...
    if (rowConfig.section && rowConfig.section !== lastSection) {
      ...
      sectionHeader.className = 'w-full text-center mt-8 mb-6';
      sectionHeader.innerHTML = `
        <div class="bg-white border-t-2 border-b-2 border-gray-300 py-4">
          <span class="text-xl font-bold text-gray-800 tracking-wide">BALCONY</span>
        </div>
      `;
    ...
    rowDiv.className = 'seat-row flex items-center gap-1 justify-center';
    ...
    // Block all seats in ground floor rows A and B only (not balcony A and B)
    for (let i = 1; i <= rowConfig.seats; i++) {
      ...
      if ((rowConfig.row === 'A' || rowConfig.row === 'B') && !rowConfig.section) {
        // Only block ground floor A and B, not balcony
...
function selectSeat(seatNumber) {
  ...
  if (seatIndex > -1) {
    ...
    if (selectedSeats.length >= seatsToBook) {
      alert('You can only select 1 seat. Please deselect your current seat first.');
      return;
    }
    ...
    try {
      ...
      if (orderData.skipPayment) {
        ...
        if (bookingData.success) {
          ...
          // If duplicate email booking, show a custom popup
          if (bookingData.error && bookingData.error.includes('already been issued for this email')) {
            document.body.removeChild(popup);
            showAlreadyBookedPopup(email);
            return;
          }
        ...
        // Show a popup if a ticket has already been issued for this email
        function showAlreadyBookedPopup(email) {
          const popup = document.createElement('div');
          popup.className = 'fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50';
          popup.innerHTML = `
            <div class="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
              <div class="text-5xl mb-4">🎟️</div>
              <h2 class="text-2xl font-bold text-gray-800 mb-2">Ticket Already Issued</h2>
              <p class="text-gray-700 mb-4">A ticket has already been issued for <span class="font-semibold text-blue-600">${email}</span>.</p>
              <button id="already-booked-close" class="mt-4 bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-6 rounded transition-colors">OK</button>
            </div>
          `;
          document.body.appendChild(popup);
          document.getElementById('already-booked-close').onclick = () => {
            document.body.removeChild(popup);
          };
        }
        return;
